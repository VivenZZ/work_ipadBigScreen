<template>
  <div class="hot">
    <div class="h100"></div>
    <ul class="goods_list">
      <li v-for="(data, index) in hotgoods" :key="data.id">
        <div class="goods_img">
...
export default {
  ...
  created () {
    let _this = this
    _this.api.post('market/product/hotlist', {}, function (result) {
      if (!result.IsError) {
        result.Data.hotgoods.forEach(e => {
  ...
  methods: {
    ...
    //   let _this = this
    //   _this.pageIndex = 1
    //   _this.api.post('/market/product/hotlist', {pageIndex: 1, pageSize: 10}, function (result) {
    //     if (!result.IsError) {
    //       _this.hotgoods = result.Data.hotgoods
    ...
    //   let _this = this
    //   _this.pageIndex++
    //   _this.api.post('/market/product/hotlist', {pageIndex: _this.pageIndex, pageSize: 10}, function (result) {
    //     if (!result.IsError) {
    //       result.Data.hotgoods.forEach(e => {
    //         _this.hotgoods.push(e)
    //       })
    //       if (result.Data.hotgoods.length <= 10) { // 当数据小于10条的时候取消加载 显示nodatatext
