<template>
  <div>
    <transition name="fade">
      <div class="goodsDetails" v-if="isShow">
        <div class="h100"></div>
        <div class="banner mpgoodsshare_banner">
          <swiper :options="swiperOption" ref="mySwiper">
            <!-- slides -->
            <swiper-slide  v-for="pic in data[i].Gallerys" :key="pic.id">
                <img v-lazy="{src: pic, error: errorImg}"  :key="pic">
            </swiper-slide>
            <div class="swiper-pagination"  slot="pagination"></div>
          </swiper>
        </div>
        <div class="content">
          <div class="info">
            <div class="left">
              <p class="name">{{data[i].Title}}</p>
              <p class="mt20">
                <span class="price" v-if="data[i].Price === data[i].PriceDiscount">￥{{data[i].Price}} </span>
                <span class="price" v-if="data[i].Price !== data[i].PriceDiscount">￥{{data[i].PriceDiscount}}</span>
                <del class="oldPrice" v-if="data[i].Price !== data[i].PriceDiscount">￥{{data[i].Price}}</del>
              </p>
            </div>
            <p class="right">
              <span class="stock mr8">{{data[i].Code}}</span>
            </p>
          </div>
          <ul class="mt40">
            <li class="mr25">规格</li>
            <li>
              <span v-for="(data, index) in Attrs.PSize" :class="info.pNumber === index ? 'current': ''"  @click="pcurrent(index)"  :key="data.id">{{data}}</span>
            </li>
          </ul>
          <ul class="mt40">
            <li class="mr25">色款</li>
            <li class="maxw">
              <!-- <span class="maxw" v-for="(data, index) in Attrs.Color" :class="info.cNumber === index ? 'current': ''"  @click="ccurrent(index)"  :key="data.id">{{data}}</span> -->
              <img  v-for="(data, index) in Attrs.Color" :class="info.cNumber === index ? 'current': ''"  @click="ccurrent(index, codes[index])"  :key="index" :src="data" alt="">
            </li>
          </ul>
          <ul class="mt40">
            <li class="mr25">描述</li>
          </ul>
          <div class="detailsInfo" v-html="detailsInfo">
          </div>

        </div>
      </div>
    </transition>
    <div class="loading" v-if="!isShow">
      <fulfilling-bouncing-circle-spinner
        :animation-duration="4000"
        :size="60"
        color="#EB4D3D"
      />
    </div>
    <tips v-if="tipsShow" :msg="tipsData" />
  </div>
</template>
<script>
import {FulfillingBouncingCircleSpinner} from 'epic-spinners'
import { swiper, swiperSlide } from 'vue-awesome-swiper'
import Tips from '@/components/tips'
export default{
  name: 'goodsDetails',
  components: {
    FulfillingBouncingCircleSpinner,
    swiper,
    swiperSlide,
    Tips
  },
  data () {
    return {
      tipsShow: false,
      tipsData: '不能再减少了',
      swiperOption: {
        pagination: {
          el: '.swiper-pagination',
          clickable: true
        }
      },
      enterMsg: '加入购物车',
      data: [
        {
          Gallerys: []
        }
      ],
      id: '',
      i: 0, // 当前选中第一个
      quantity: 1,
      Attrs: {
        PSize: [],
        Color: []
      },
      codes: [],
      info: {
        pNumber: -1,
        cNumber: -1,
        activeP: '',
        activeC: ''
      },
      UnionID: '',
      code: -1,
      isShow: false,
      city: '',
      detailsInfo: '',
      errorImg: '',
      share: {
        title: '',
        img: '',
        desc: ''
      }
    }
  },
  watch: {
    '$route' (to, from) {
    }
  },
  computed: {
    swiper () {
      return this.$refs.mySwiper.swiper
    }
  },
  methods: {
    showTips (m) {
      let _this = this
      if (!this.tipsShow) {
        this.tipsData = m
        this.tipsShow = true
        setTimeout(function () {
          _this.tipsShow = false
        }, 2000)
      }
    },
    enlarge () {
      // window.location.href = 'http://0.0.0.0:8081/#/goodsImage/' + this.id
      this.sdk.bridge.openNewPage(this.common.url + '/#/goodsImage/' + this.id, 'nobar', '1111')
    },
    pcurrent (i) {
      if (this.info.pNumber === i) {
        this.info.pNumber = -1
        this.info.cNumber = -1
        this.code = -1
        this.info.activeC = ''
        this.info.activeP = ''
      } else {
        this.info.pNumber = i
        this.info.cNumber = -1
        this.code = 0
        this.info.activeC = ''
        this.info.activeP = this.Attrs.PSize[i]
        this.Attrs.Color = []
        this.codes = []
        this.data.forEach(r => {
          if (r.Attrs.PSize === this.info.activeP) {
            this.Attrs.Color.push(r.Gallerys[0])
            this.codes.push(r.Code)
          }
        })
      }
    },
    ccurrent (i, code) {
      if (this.info.cNumber === i) {
        this.info.cNumber = -1
        this.code = 0
        this.info.activeC = ''
      } else {
        this.info.cNumber = i
        this.info.activeC = this.Attrs.Color[i]
        // if (this.info.pNumber !== -1) {
        //   this.data.forEach((e, i) => {
        //     if (e.Attrs.PSize === this.info.activeP && e.Gallerys[0].Img_Url === this.info.activeC) {
        //       this.i = i
        //       this.code = this.data[i].Code
        //     }
        //   })
        // }
        if (this.info.pNumber !== -1) {
          this.data.forEach((e, i) => {
            if (e.Code === code) {
              this.i = i
              this.code = code
              let _this = this
              document.title = this.data[this.i].Title
              // 分享
              _this.share.title = _this.data[i].Title
              _this.share.img = 'http:' + _this.data[i].Gallerys[0]
              _this.share.desc = '规格：' + _this.data[i].Attrs.PSize + ';色款：' + _this.data[i].Attrs.Color + '/' + _this.data[i].Attrs.Material + ';'
              _this.wxConfig.wxShowMenu(_this.share.title, _this.share.img, _this.share.desc)
              this.$nextTick(() => {
                this.swiper.updateProgress()
              })
            }
          })
        }
      }
    },
    reduce () {
      if (this.quantity > 1) {
        this.quantity--
      } else {
        this.showTips('不能减少了')
      }
    },
    add () {
      if (this.quantity < 999) {
        if (this.data[this.i].isStock === 1) {
          if (this.data[this.i].stock > this.quantity) {
            this.quantity++
          } else {
            this.showTips('库存不足了')
          }
        }
        if (this.data[this.i].isStock === 2) {
          this.quantity++
        }
      }
    }
  },
  created () {
    let _this = this
    this.id = this.$route.params.id
    this.code = this.$route.params.code
    let mid = this.$route.params.mid
    let sid = this.$route.params.sid
    this.errorImg = this.common.url + '/static/images/onerror480.jpg'
    // _this.api.post2('goods/getsharespu', {gid: this.id, mid: mid, sid: sid}, function (result) {
    //   if (!result.IsError) {
    //     console.log(result)
    // 测试
    _this.api.post2('goods/getsharespu', {gid: this.id, mid: mid, sid: sid}, function (result) {
      if (!result.IsError) {
        // _this.UnionID = result.Data.Goods.UnionID
        _this.data = result.Data.skus
        document.title = _this.data[_this.i].Title
        _this.detailsInfo = result.Data.goods.Description
        _this.data.forEach(r => {
          _this.Attrs.PSize.push(r.Attrs.PSize)
          // _this.Attrs.Color.push(r.Attrs.Material + '/' + r.Attrs.Color)
          if (r.Code === _this.code) {
            _this.info.activeP = r.Attrs.PSize
            _this.info.activeC = r.Gallerys[0]
          }
        })
        _this.Attrs.PSize = Array.from(new Set(_this.Attrs.PSize)) // 去重
        _this.data.forEach(r => {
          if (r.Attrs.PSize === _this.info.activeP) {
            _this.Attrs.Color.push(r.Gallerys[0])
            _this.codes.push(r.Code)
          }
        })
        _this.Attrs.PSize.forEach((e, i) => {
          if (_this.info.activeP === e) {
            _this.info.pNumber = i
          }
        })
        _this.Attrs.Color.forEach((e, i) => {
          if (_this.info.activeC === e) {
            _this.info.cNumber = i
          }
        })
        _this.data.forEach((e, i) => {
          if (e.Attrs.PSize === _this.info.activeP && e.Gallerys[0] === _this.info.activeC) {
            _this.i = i
            _this.code = _this.data[i].Code
            // 分享
            _this.share.title = _this.data[i].Title
            _this.share.img = 'http:' + _this.data[i].Gallerys[0]
            _this.share.desc = '规格：' + _this.data[i].Attrs.PSize + ';色款：' + _this.data[i].Attrs.Color + '/' + _this.data[i].Attrs.Material + ';'
            _this.wxConfig.wxShowMenu(_this.share.title, _this.share.img, _this.share.desc)
          }
        })
        _this.isShow = true
      }
    })
  }
}
</script>
<style lang="less">
.mpgoodsshare_banner{
  .swiper-pagination-bullet-active{
    opacity: 1;
    background: #007aff  !important;
  }
  .swiper-pagination-bullet {
      width: .2rem;
      height: .2rem;
      display: inline-block;
      border-radius: 100%;
      background: #000;
      margin: 0 .066667rem  !important;
  }
}
</style>

<style lang="less" scoped>
.goodsDetails{
  .h{
    width: 100%;
    height: 0.957031rem;
  }
  .jion{
    font-size: .332031rem;
    height: .957031rem;
    line-height: .957031rem;
    color: #fff;
    background-color: #333;
    width: 100%;
    position: fixed;
    bottom: 0;
    left: 0;
    text-align: center;
  }
  .content{
    width: 8.789063rem;
    margin: .488281rem auto 0;
    .ov{
      overflow: hidden;
    }
    input{
      color: #333;
      text-align: center;
    }
    .number{
      margin: 0 4/102.4rem;
      width: 130/102.4rem;
      height: 50/102.4rem;
      border: #999 1px solid;
      -moz-user-select: none;
      -webkit-user-select: none;
    }
    .btn{
      display: inline-block;
      width: 50/102.4rem;
      height: 50/102.4rem;
      border: #999 1px solid;
      border-radius: 5px;
      img{
        width: 100%;
        height: 100%;
      }
    }
    .mt40{
      padding: 0.5rem 0;
      border-bottom: 1px solid rgba(0, 0, 0, 0.1);
      li{
        line-height: .533333rem;
      }
    }
    ul{
      overflow: hidden;
      li{
        font-size: .373333rem;
        color: #333;
        line-height: 42/102.4rem;
        max-width: 600/102.4rem;
        float: left;
        span{
          overflow: hidden;
          text-overflow:ellipsis; // 文本溢出显示省略号
          white-space:nowrap; // 文本不会换行（单行文本溢出）
          display: inline-block;
          padding: 0 5/102.4rem;
          border: 1px solid #999;
          border-radius: 4px;
          margin: 0 15/102.4rem 15/102.4rem 0;
          padding: 0 .133333rem;
          max-width: 70%;
        }
        .maxw{
          max-width: 1.953125rem;
        }
        img{
          width: 100/102.4rem;
          height: 100/102.4rem;
          border: 1px solid #999;
          margin-right: 0.2rem;
          display: inline-block;
        }
        .current{
          color: #56565A;
          border: 1px solid #56565A;
        }
      }
      .mr25{
        margin-right: 25/102.4rem;
      }
    }
    .info{
      overflow: hidden;
      border-bottom: 1px solid rgba(0, 0, 0, 0.1)
    }
    .mt20{
      margin-top: .195313rem;
      margin-bottom: .195313rem;
    }
    .left{
      float: left;
      color: #333;
      font-size: .195313rem;
    }
    .right{
      float: right;
    }
    .name{
      font-size: .48rem;
      color: #333;
      font-weight: bold;
    }
    .price{
      color: #CF4D3D;
      font-size: .48rem;
      font-family: PingFangSC-Semibold, sans-serif;
    }
    .stock{
      display: inline-block;
      color: #999;
      font-size: .346667rem;
    }
    .oldPrice{
      color: #B0B0B0;
      font-size: .346667rem;
    }
  }
  .banner{
    width: 100%;
    height: 6.4rem;
  }
  .swiper-slide {
    text-align: center;
    font-size: 18px;
    width: 100%;
    height: 100%;
    margin: 0 2px;
  }
  .swiper-slide img {
    width: auto;
    height: 6.4rem;
  }
  html,body{
    width: 100%;
    height: 100%;
    position: relative;
  }
  .h100{
    height: .976563rem;
  }
  .top{
    position: fixed;
    top: 0;
    left: 0;
    height: .859375rem;
    width: 100%;
    background-color: #fff;
    line-height: .957031rem;
    overflow: hidden;
    border-radius: 3px 3px 0 0;
    .back{
      float: left;
      width: .859375rem;
      height: .859375rem;
      background:url('/static/images/back.png') center center no-repeat;
      background-size: 100%;
      margin-left: .292969rem;
    }
    .share{
      float: right;
      width: .859375rem;
      height: .859375rem;
      background:url('/static/images/share.png') center center no-repeat;
      background-size: 100%;
      margin-right: .292969rem;
    }
  }
  .current{
    color: #EB4D3D;
    border-color: #EB4D3D;
  }
  .mr8{
    margin-right: 8/102.4rem;
  }
  .detailsInfo{
    margin-top: 0.5rem;
  }
}

</style>
